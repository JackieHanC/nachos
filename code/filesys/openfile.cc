...
{
    hdr->setLastVisitTime();
    hdr->setLastChangeTime();
    hdr->WriteBack(this->sector);

    printf("OpenFile::Write\n");
...
OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    if ((position + numBytes) > fileLength) {
        ...
        if (fileSystem->ReAllocateFile(this, position + numBytes - fileLength)){
            printf("ReAllocateFileSize success\n");
        } else {
            printf("ReAllocateFileSize fail\n");
            numBytes = fileLength - position;
        }
    }
    printf("Writing %d bytes at %d, from file of length %d.\n", 
        numBytes, position, fileLength);
